...
export class AuthService {
  ...
  async confirmPassRecovery(confirmPassRecovery: ConfirmPassRecoveryDto) {
    ...
    const { send_status } = await sendSMS(
      `+${user.profile.phone}`,
      ` ${totp} es su código de verificación de Chamoy`,
    );
  ...
  async otpPassRecovery(otpPassRecoveryDto: OtpPassRecoveryDto) {
    ...
    const isValid = verifyOTP(
      { phone: user.profile.phone, password: user.password },
      otp,
    );
  ...
  async setNewPassword(setNewPasswordDto: SetNewPasswordDto) {
    const { newPassword, path } = setNewPasswordDto;
    const user_id = EncryptionService.decrypt(path);
    const user = await this.usersService.getUserById(user_id);
    if (!user) {
      throw new HttpException(
    ...
    await this.usersService.setNewPassword(user_id, newPassword);

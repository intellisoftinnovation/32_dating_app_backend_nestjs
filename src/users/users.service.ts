...
export class UsersService {
  ...
  async createUser(createUserDto: CreateUserDto) {
    createUserDto.password = bycrypt.hashSync(
    ...
    await this.metaDataModel.findByIdAndUpdate(user.metaData, {
      $set: { active_session: token },
    });
  ...
  async getSelfUser(id: string) {
    ...
    if (!user)
      throw new HttpException(
        { message: `User ${id} not found`, statusCode: HttpStatus.NOT_FOUND },
  ...
  async updateUser(
    ...
    if (!Object.keys(updateUserDto).length)
      throw new HttpException(
        { message: `Nothing to update`, statusCode: HttpStatus.BAD_REQUEST },
        HttpStatus.BAD_REQUEST,
      );

    if (name)
      await this.userModel.updateOne({ _id: user._id }, { $set: { name } });
    if (email)
    ...
    if (gender) {
      await this.profileModel.updateOne(
        { _id: user.profile },
        { $set: { gender, genderVerified: false } },
      );
    }
    ...
    if (photos) {
      if (photos.length > 6)
        throw new HttpException(
          { message: `You can only to have 6 photos` },
          HttpStatus.BAD_REQUEST,
        );
      await this.profileModel.updateOne(
        { _id: user.profile },
  ...
  async findForLogin(findForLogin: FindForLoginDto) {
    ...
    // console.log(user)
    if (!user || user.metaData.accountStatus == AccountStatus.DELETED)
      throw new HttpException(
        { message: `User with ${email} dont exists` },
        HttpStatus.NOT_FOUND,
      );
    if (user.metaData.accountStatus == AccountStatus.SUSPENDED)
      throw new HttpException(
        { message: `User with ${email} is suspended` },
  ...
  async verifyGender(id: string) {
    ...
    await user.populate('profile');
    await this.profileModel.findByIdAndUpdate(user.profile, {
      $set: { genderVerified: true },
    });
    return true;
  }
  ...
  async findForPassRecovery(email: string) {
    ...
    await user.populate('metaData');
    await user.populate('profile');
    if (user.metaData.accountStatus == AccountStatus.DELETED)
      throw new HttpException(
  ...
  async getUserById(id: string) {
    ...
    if (!user)
      throw new HttpException(
        { message: `User with ${id} dont exists` },
        HttpStatus.NOT_FOUND,
      );
    await user.populate('metaData');
    await user.populate('profile');
    ...
    if (user.metaData.accountStatus == AccountStatus.SUSPENDED)
      throw new HttpException(
        { message: `User with ${id} is suspended` },
        HttpStatus.FORBIDDEN,
      );
    return user;
  }
  ...
  async getSelfPreferences(id: string) {
    ...
    if (!user.preference) {
      ...
      await this.userModel.findByIdAndUpdate(user._id, {
        preference: tempPreferences._id,
      });
  ...
  async updateSelfPreferences(
    ...
    if (!Object.keys(updatePreferenceDto).length)
      throw new HttpException(
  ...
  async seedUsers(many: number) {
    ...
    for (let i = 0; i < seedUsers.length; i++) {
      ...
      try {
        ...
        const tempMetadata = await this.metaDataModel.create({
          userId: tempUser._id,
          ...metaData,
        });
        const tempProfile = await this.profileModel.create({
          userId: tempUser._id,
          ...profile,
        });
        await this.userModel.findByIdAndUpdate(tempUser._id, {
          metaData: tempMetadata._id,
  ...
  async getAllUsers(id: string, getUsersDto: GetUsersDto) {
    ...
    if ((altura_min || altura_max) && !(altura_min && altura_max))
      throw new HttpException(
        {
          message: 'Altura range is invalid .',
          statusCode: HttpStatus.BAD_REQUEST,
        },
        HttpStatus.BAD_REQUEST,
      );
    if (altura_min && altura_max && altura_max < altura_min)
      throw new HttpException(
        {
          message: 'Altura range is invalid ..',
          statusCode: HttpStatus.BAD_REQUEST,
    ...
    const data = await this.userModel
      .find({})
      .lean()
      .populate('profile', '-_id -description -__v -socialNetworks')
      .populate('metaData', 'createdAt accountStatus -_id');
    ...
    const dataFiltered = data.reduce((acc, user) => {
      ...
      if (selfLocation && user.profile.geoLocations) {
        userDistance = HaverSine(
          ...
          user.profile.geoLocations.latitude,
    ...
    if (sortBy == SortBy.HOT)
      dataFiltered.sort((a, b) => b.profile.request - a.profile.request);

    const startIndex = (page - 1) * size;
    const endIndex = page * size;

    const paginatedData = paggination
      ? dataFiltered.slice(startIndex, endIndex)
      : dataFiltered;
  ...
  async getUserProfile(id: string, idInToken: string) {
    ...
    if (!selfUser)
      throw new HttpException(
        { message: `User with id ${idInToken} not found` },
        HttpStatus.NOT_FOUND,
      );
    await user.populate('profile');
    await user.populate('metaData');
    ...
    if (user.metaData.accountStatus == AccountStatus.SUSPENDED)
      throw new HttpException(
        { message: `User with id ${id} is suspended` },
        HttpStatus.FORBIDDEN,
      );

    const solicitud1 = await this.matchRequestService.getMatchRequestByFromTo(
      idInToken,
      id,
    );
    const solicitud2 = await this.matchRequestService.getMatchRequestByFromTo(
      id,
      idInToken,
    );
    let distance = -1;
    ...
    return {
      message: `User with id ${id} profile fetched`,
      profile: {
        name: user.name,
  ...
  async deleteUser(idInToken: string) {
    ...
    if (!user)
      throw new HttpException(
        {
          message: `User ${idInToken} not found`,
    ...
    await this.metaDataModel.findByIdAndUpdate(user.metaData, {
      $set: { active_session: '', accountStatus: AccountStatus.DELETED },
    });
  ...
  async createComplaint(
    ...
    if (!subject)
      throw new HttpException(
        { message: `User with id ${createComplaintDto.subject} not found` },
        HttpStatus.NOT_FOUND,
      );
  ...
  async actionComplaint(id: string, complaintActionDto: ComplaintActionDto) {
    ...
    if (action === ComplaintAction.ACTIVATE) {
      ...
      try {
        await this.firebaseAdminService.sendNotificationToDevice(
          user.profile.fcmToken,
          {
            title: '✨ Ya puedes volver a Chamoy',
            body: 'Hemos resuelto tu apelación.',
          },
        );
      } catch (error) {
        console.log(error);
      ...
      if (updatedUser.metaData.accountStatus !== AccountStatus.ACTIVE) {
        throw new HttpException(
          { message: `Failed to update user account status to ACTIVE` },
    ...
    switch (action) {
      ...
      case ComplaintAction.SUSPEND:
        await this.metaDataModel.findByIdAndUpdate(user.metaData, {
          $set: { accountStatus: AccountStatus.SUSPENDED },
        });
      ...
      case ComplaintAction.CHANGE_GENDER:
        await this.updateUser(id, {
          gender:
            user.profile.gender == Gender.MALE ? Gender.FEMALE : Gender.MALE,
        });
        try {
          await this.firebaseAdminService.sendNotificationToDevice(
            user.profile.fcmToken,
  ...
  async getUsersCreateAt() {
    const users = await this.userModel
      ...
      .select('inc_id')
      .populate('metaData');
    const usersCreateAt = users.map((u) => ({
      inc_id: u.inc_id,
  ...
  async getStats() {
    ...
    const users_this_month = await this.metaDataModel
      .find({
        createdAt: {
    ...
    for (let i = 0; i < allSubscriptions.length; i++) {
      ...
      if (
        ...
        ids.includes(subscription.external_reference.toString())
      ) {
